...
class CategoryListController extends StateNotifier<CategoryState> {
  ...
  //#region ==================== accessor ====================
  // List<CategoryState>? get lstPlan => state.resultList;
  ...
  Future<void> setMasterData() async {
    ...
    // await api.getCategory("", CategoryType.room.id);
    // masterRoomType = await api.gqlSearchRoomTypeMaster();
    // await api.gqlSearchCancellationPolicyMaster("");

    // await api.getqu("", CategoryType.room.id);
  }

  // List<CategoryState>? get filteredCancellationPolicy {
  ...
  // Iterable<CategoryState> list = [...lstPlan!];
